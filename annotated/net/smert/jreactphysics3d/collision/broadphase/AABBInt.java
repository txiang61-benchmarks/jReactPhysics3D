...
public class AABBInt {
    ...
    public final long[] min = new @Dimensionless long @Dimensionless [((@Dimensionless int) (3))];

    // Maximum values on the three axis
    public final long[] max = new @Dimensionless long @Dimensionless [((@Dimensionless int) (3))];

    // Constructor that takes an AABB as input
    public AABBInt(AABB aabb) {
        min[((@Dimensionless int) (0))] = Utils.encodeFloatIntoInteger(aabb.getMin().getX());
        min[((@Dimensionless int) (1))] = Utils.encodeFloatIntoInteger(aabb.getMin().getY());
        min[((@Dimensionless int) (2))] = Utils.encodeFloatIntoInteger(aabb.getMin().getZ());

        max[((@Dimensionless int) (0))] = Utils.encodeFloatIntoInteger(aabb.getMax().getX());
        max[((@Dimensionless int) (1))] = Utils.encodeFloatIntoInteger(aabb.getMax().getY());
        max[((@Dimensionless int) (2))] = Utils.encodeFloatIntoInteger(aabb.getMax().getZ());
    }
