...
public class HingeJointInfo extends JointInfo {
    ...
    // Constructor
    public HingeJointInfo(@Dimensionless JointType constraintType) {
        super(constraintType);
    }

    // Constructor without limits and without motor
    public HingeJointInfo(@Dimensionless RigidBody rigidBody1, @Dimensionless RigidBody rigidBody2,
            @Dimensionless
            Vector3 initAnchorPointWorldSpace, @Dimensionless Vector3 initRotationAxisWorld) {
        super(rigidBody1, rigidBody2, JointType.HINGEJOINT);
        anchorPointWorldSpace = initAnchorPointWorldSpace;
        rotationAxisWorld = initRotationAxisWorld;
        isLimitEnabled = true;
        isMotorEnabled = false;
        minAngleLimit = - ((@Dimensionless float) (1.0f));
        maxAngleLimit = ((@Dimensionless float) (1.0f));
        motorSpeed = ((@Dimensionless float) (0.0f));
        maxMotorTorque = ((@Dimensionless float) (0.0f));
    }
    ...
    public HingeJointInfo(@Dimensionless RigidBody rigidBody1, @Dimensionless RigidBody rigidBody2,
            ...
            Vector3 initAnchorPointWorldSpace, @Dimensionless Vector3 initRotationAxisWorld,
            @Dimensionless
            float initMinAngleLimit, @Dimensionless float initMaxAngleLimit) {
        super(rigidBody1, rigidBody2, JointType.HINGEJOINT);
        anchorPointWorldSpace = initAnchorPointWorldSpace;
        rotationAxisWorld = initRotationAxisWorld;
        isLimitEnabled = true;
